...
void ClothComp::BuildDescriptors(CD3DX12_CPU_DESCRIPTOR_HANDLE hCpuDescriptor,
	CD3DX12_GPU_DESCRIPTOR_HANDLE hGpuDescriptor,
	UINT descriptorSize)
{
	D3D12_SHADER_RESOURCE_VIEW_DESC compsrvDesc = {};
	compsrvDesc.Shader4ComponentMapping = D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING;
	compsrvDesc.Format = DXGI_FORMAT_R32_FLOAT;
	compsrvDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE2D;
	...
	md3dDevice->CreateShaderResourceView(mPrevSol.Get(), &compsrvDesc, hCpuDescriptor);
	md3dDevice->CreateShaderResourceView(mCurrSol.Get(), &compsrvDesc, hCpuDescriptor.Offset(1, descriptorSize));
	md3dDevice->CreateShaderResourceView(mNextSol.Get(), &compsrvDesc, hCpuDescriptor.Offset(1, descriptorSize));
...
ClothComp::ClothComp(ID3D12Device* device, ID3D12GraphicsCommandList* cmdList,
...
void ClothComp::Update(const GameTimer& gt,
...
UINT ClothComp::DesciptorCount()const
{
	return 6;
}

UINT ClothComp::RowCount()const
{
	return mNumRows;
}

UINT ClothComp::ColumnCount()const
{
	return mNumCols;
}

UINT ClothComp::VertexCount()const
{
	return mVertexCount;
}

UINT ClothComp::TriangleCount()const
{
	return mTriangleCount;
}

float ClothComp::Width()const
{
	return mNumCols * mSpatialStep;
}

float ClothComp::Depth()const
{
	return mNumRows * mSpatialStep;
}

float ClothComp::SpatialStep()const
{
	return mSpatialStep;
}

CD3DX12_GPU_DESCRIPTOR_HANDLE ClothComp::DisplacementMap()const
{
	return mCurrSolSrv;
}

void ClothComp::BuildResources(ID3D12GraphicsCommandList* cmdList)
{
	//create 3 texture resources
	...
	std::vector<float> initData(mNumCols*mNumRows, 0.0f);
